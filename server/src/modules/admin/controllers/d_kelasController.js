...
module.exports = {
	...
	add: (req, res) => {
		d_kelas.add(req.con, req.body, (err) => {
			if(err) throw err
            return res.json({error : false, message :'Berhasil tambah data daftar kelas'})
		})
	},
    ...
    update: (req, res) => {
		d_kelas.update(req.con, req.body, req.params.d_kelas_id, res, (err) => {
			if(err) throw err
            return res.json({error : false, message :'Berhasil ubah data daftar kelas'})
		})
	},
	...
	delete: (req, res) => {
		d_kelas.delete(req.con, req.params.d_kelas_id, res, (err) => {
            ...
            return res.json({error : false, message :'Berhasil hapus data daftar kelas'})
		})
	}
}