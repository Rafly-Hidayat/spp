...
module.exports = {
    ...
    register: (req, res) => {
        con.beginTransaction(err => {
            ...
            con.query('SELECT admin_email FROM admin WHERE admin_email = ?', [post.admin_email], (err, rows) => {
                if (err) res.send(err.sqlMessage, 400)
    ...
    login: (req, res) => {
        ...
        con.query(`SELECT * FROM admin WHERE admin_password = ? AND admin_email = ?`, [post.admin_password, post.admin_email], (err, rows) => {
            if (err) res.send(err.sqlMessage, 400)
